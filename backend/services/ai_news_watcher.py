...
class NewsWatcher:
    ...
    async def run_check(self, manual: bool = False) -> Dict:
        ...
        if alert and doc["severity"] in ("medium", "high"):
            ...
            # festen Uhrzeiten): analysiert Ergebnis vs. Erwartung + Prognose.
            # Token-Optimierung: nur NEUE Ereignisse triggern – ein bereits tief
            # analysiertes Event (z.B. dieselben FOMC-Minutes) triggert nicht erneut.
            if should_trigger_deep(doc["severity"],
                ...
                if titles and not new_titles:
                    ...
                    logger.info(f"news watcher: {doc['severity'].upper()}-Impact -> "
                                "Tiefenanalyse (Event-getriggert) ausgelöst")
                    asyncio.create_task(self.engine.run_deep_analysis(
                        manual=False, trigger="news", news_event=doc))
